...
#include <Network/Telosb/serialprotocol.h>
...
serial_source open_serial_source(const char *device, int baud_rate,
		int non_blocking, void (*message)(serial_source_msg problem))
...
static void push_protocol_packet(serial_source src, uint8_t type,
 ...
 */
{
	if (src->recv.bufpos >= src->recv.bufused) {
		for (;;) {
			int n = serial_read(src, non_blocking, src->recv.buffer,
					sizeof src->recv.buffer);
...
static int write_framed_packet(serial_source src, uint8_t packet_type,
		uint8_t first_byte, const uint8_t *packet, int count);
 ...
 */
{
	...
	src->send.seqno++;
	if (write_framed_packet(src, P_PACKET_ACK, src->send.seqno, packet, len)
			< 0) {
		return -1;
	}
	/*
